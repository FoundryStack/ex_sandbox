defmodule ExSandbox.Egress.LaunchDecisionsTest do
  ...
  describe "which program is actually exec'd" do
    test "the plan's own head becomes the program, not the original one" do
      ...
      assert prog == "ip"
      assert args |> Enum.take(2) == ["netns", "exec"]
    ...
    test "the netns name matches the /30 the policy is keyed by" do
      ...
      # identity the pool enforces against -- the sandbox would be in namespace
      # A while its policy was filed under B.
      assert Enum.at(args, 2) == LaunchPlan.netns_name({10, 0, 0, 8})
    end
  end
end
